...
module Ddr::Batch
  ...
  RSpec.shared_examples "an invalid metadata file" do
  ...
  RSpec.shared_examples "a successful metadata file processing" do
    it "should create a batch with an appropriate UpdateBatchObject" do
      ...
      expect(@datastream.operation).to eq(Ddr::Batch::BatchObjectDatastream::OPERATION_ADDUPDATE)
      expect(@datastream.payload_type).to eq(Ddr::Batch::BatchObjectDatastream::PAYLOAD_TYPE_BYTES)
      expect(RDF::Reader.for(:ntriples).new(@datastream.payload)).to be_isomorphic_with(RDF::Reader.for(:ntriples).new(expected_md))
    end
  end

  RSpec.describe MetadataFile, :type => :model, :metadata_file => true do
    ...
    context "validation" do
      ...
      context "metadata file not parseable with profile" do
        before { metadata_file.update(:metadata => File.new(File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'mapped_tab.txt'))) }
      ...
      context "metadata file invalid attribute names"do
        context "invalid metadata header" do
          before { metadata_file.update(:metadata => File.new(File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'descmd_csv_invalid_column.csv'))) }
    ...
    context "successful processing", batch: true do
      ...
      context "cdm export metadata file" do
        let(:delimited_file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'mapped_tab.txt') }
        let(:options) do
          {
            :csv => {
              :col_sep => "\t",
      ...
      context "desc metadata csv file" do
        let(:delimited_file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'descmd_csv.csv') }
        let(:options) do
          {
            :csv => {
              :col_sep => ",",
