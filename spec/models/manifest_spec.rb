...
module Ddr::Batch
  ...
  RSpec.describe Manifest, type: :model, batch: true do
    ...
    context "validate" do
      ...
      context "valid" do
        ...
        context "manifest with files" do
          let(:manifest) { Manifest.new(File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'manifest_with_files.yml')) }
          before { manifest.manifest_hash['basepath'] = File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures') }
        ...
        context "checksums" do
          ...
          context "checksum file" do
            ...
            context "not XML document" do
              let(:file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'manifest_hash.txt') }
            ...
            context "node_xpath" do
              context "not specified and default xpath absent" do
                let(:file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'metadata.xml') }
              ...
              context "specified but absent" do
                let(:file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'checksums.xml') }
            ...
            context "identifier_element" do
              context "not specified and default xpath absent" do
                let(:file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'incorrect_checksums.xml') }
            ...
            context "type xpath" do
              context "not specified and default type xpath absent" do
                let(:file) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'checksums_bad_elements.xml') }
    ...
    context "load yaml manifest file" do
      let(:manifest) { Manifest.new(File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'manifest.yml')) }
      let(:yaml_hash) { eval File.open(File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'manifest_hash.txt')) { |f| f.read } }
    ...
    context "methods" do
      ...
      context "checksums" do
        ...
        context "checksums XML document" do
          let(:checksums_filepath) { File.join(Ddr::Batch::Engine.root.to_s, 'spec', 'fixtures', 'checksums.xml') }
